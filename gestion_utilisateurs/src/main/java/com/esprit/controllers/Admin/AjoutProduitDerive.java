...
public class AjoutProduitDerive {
    ...
    private void retourAccueil() {
        System.out.println("abdenour - Tentative de fermeture du popup...");
        try {
            // Obtenir la Stage (fenêtre) à partir du bouton qui a été cliqué
            Stage stage = (Stage) btnRetour.getScene().getWindow();
            stage.close(); // Ferme la fenêtre actuelle

            System.out.println("✅ Pop-up fermé avec succès.");

            // Si après avoir fermé le pop-up, vous voulez revenir à une autre scène
            // (par exemple, la scène qui a ouvert ce pop-up), vous devriez
            // réactiver ou afficher cette scène, mais pas ici directement
            // car cette méthode est censée juste "retourner" et fermer.

        } catch (Exception e) {
            System.err.println("❌ Erreur lors de la fermeture du pop-up : " + e.getMessage());
            e.printStackTrace();
        }
    }
    /*@FXML
    private void retourAccueil() {
        System.out.println("abdenour");
        try {
